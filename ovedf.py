...
class EDFdata:
        ...
        try:
            for line in fedf:
                ...
                if line.startswith("pixelsize_x"):
                    self.pixSXmm = float(line[iigual + 1: ipcoma].strip()) / 1000.0
                    log.debug("pixelsize_x=" + str(self.pixSXmm))
                if line.startswith("pixelsize_y"):
                    self.pixSYmm = float(line[iigual + 1: ipcoma].strip()) / 1000.0
    ...
    def readBINFile(self, binFilename):
        ...
        log.info(
            "dimx=%d dimy=%d xcen=%.2f ycen=%.2f pixSXmm=%.2f pixSYmm=%.2f distMDmm=%.2f wavelA=%.4f"
            % (
                self.dimx,
                self.dimy,
                self.xcen,
...
class IntegOptions:
    ...
    def readINPFile(self, inpFilename):
        ...
        log.debug(
            "tilt=%.6f rot=%.6f sT=%.6f cT=%.6f sR=%.6f cR=%.6f"
            % (
                self.tiltRotation,
                self.angleTilt,
                self.sintilt,
...
def calc2tDeg(integOpts, edfdata, rowY, colX):
    ...
    distPix = (integOpts.distMDmm / edfdata.pixSXmm) / costilt
...
def getAzim(integOpts, edfdata, rowY, colX):
    if rowY == int(edfdata.ycen + 0.5) and colX == int(edfdata.xcen + 0.5):
        return 0
...
if __name__ == "__main__":
    ...
    welcomeMsg = (
        "=== MSPD EDF IMAGE INTEGRATION ver." + __version__ + " by OV === " + st
    )
    ...
    parser.add_argument(
        ...
        "--fit2d",
        action="store_true",
        help="fit2D convention for tilt/rot (default is d2Dplot convention)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        ...
        "--suffix",
        default="",
        help="suffix for the output names (before the dat extension)",
    )
    ...
    hasParFile = True
    if (
        args.par is not None
    ):  # otherwise default parameters (image headers) will be used
    ...
    for i in range(len(files)):
        ...
        # Integration options
        t2in = calc2tDeg(
            integOpts, edf, integOpts.ycen, integOpts.xcen + integOpts.inRadi
        )
        t2fin = calc2tDeg(
            integOpts, edf, integOpts.ycen, integOpts.xcen + integOpts.outRadi
        )
        ...
        print(
            " CenX(px)=%.3f CenY(px)=%.3f Dist(mm)=%.3f Wave(A)=%.4f PixSX(mm)=%.4f"
            % (
                integOpts.xcen,
                integOpts.ycen,
                integOpts.distMDmm,
        ...
        if integOpts.fit2d:
            print(
                " TiltRot(º)=%.3f AngTilt(º)=%.3f startAzim(º)=%.4f endAzim(º)=%.4f [fit2d convention: 0 at 3h CW+]"
                % (
                    integOpts.tiltRotationIN,
                    integOpts.angleTilt,
                    integOpts.startAzim,
                    integOpts.endAzim,
                )
            ...
            print(
                " TiltRot(º)=%.3f AngTilt(º)=%.3f startAzim(º)=%.4f endAzim(º)=%.4f [d2dplot convention: 0 at 12h CW+]"
                % (
                    integOpts.tiltRotationIN,
                    integOpts.angleTilt,
                    integOpts.startAzim,
        ...
        print(
            " Inner/Outer Radius(Px)=%d %d (º) x/y bin= %d %d azimBins=%d radialBins=%d bkg=%.2f"
            % (
                integOpts.inRadi,
                integOpts.outRadi,
                integOpts.xbin,
                integOpts.xbin,
                integOpts.azimBins,
                integOpts.radialBins,
                integOpts.subadu,
            )
        )
        print(
            " T2ini(º)=%.4f step(º)=%.4f T2end(º)=%.4f"
            % (t2in + step / 2, step, t2fin - step / 2)
        )
        if isMask:
            print(" Mask file= %s" % (integOpts.maskf))

        log.info(" tiltRot internally used = %f" % integOpts.tiltRotation)
        log.info(
            " startAzim endAzim internally used (ref. 12h CW+) = %f %f"
            % (startAzim, endAzim)
        )
        ...
        if args.plot:
            ...
            def format_coord(x, y):
                ...
                if col >= 0 and col < numcols and row >= 0 and row < numrows:
                    ...
                    return (
                        "col_x=%.1f, row_y=%.1f, Intensity=%d, t2=%.3f, azim=%.2f"
                        % (x, y, inten, t2, azim)
                    )
                else:
        ...
        for y in range(edf.dimy):
            for x in range(edf.dimx):
                ...
                if isMask:
                    if maskbin.intenXY.item(y, x) < 0:
                        continue

                azim = edf.azim.item(y, x)

                if endAzim < startAzim:
                    if (azim < startAzim) and (azim > endAzim):
                        continue
                else:
                    if (azim < startAzim) or (azim > endAzim):
                        continue
                ...
                if integOpts.azimBins > 1:
                    for k in range(integOpts.azimBins):
                        if azimFin[k] < azimIni[k]:
                            if (azim < azimIni[k]) and (azim > azimFin[k]):
                                continue
                        else:
                            if (azim < azimIni[k]) or (azim > azimFin[k]):
                                continue
                        azimPos = k
                        break
                    if azimPos < 0:
                ...
                patts[azimPos].Iobs[p] = (
                    patts[azimPos].Iobs[p] + inten + integOpts.subadu
                )
        ...
        for k in range(integOpts.azimBins):
            ...
            fout.write("#%s  COM = %s\n" % (fname, edf.comment))
            fout.write(
                "#I vs. 2Theta [deg] Azim: %.2f %.2f Radial %d %d Tilts: %.2f %.2f Dist: %.2f Wave: %.4f Cen: %.2f %.2f Pix: %.2f Bin: %d\n"
                % (
                    integOpts.startAzim,
                    integOpts.endAzim,
                    integOpts.inRadi,
            ...
            for j in range(size):
                ...
                # lorentz correction
                lorfact = 1.0 / ((math.cos(math.radians(t2))) ** 3)
                ...
                if args.noesd is False:
                    esd = math.sqrt(inten / patts[k].npix[j])
                    # write the line
                    fout.write(
                        " %.7E  %.7E %.7E\n" % (round(t2, 4), inten, esd)
                    )  # round to 4 decimals (ffops uses 3)
                else:
